...
df=pd.read_csv('https://raw.githubusercontent.com/amanmalali/Stride/master/one_hump.csv')
X=df['x']
Y=df['y']
model=Sequential() #Define the model to be sequential in nature
#Specify the input layer with one node and the hidden layer with two nodes
model.add(Dense(2,input_dim=1))  #Performs a0=W0.x+b0

model.add(Activation('sigmoid')) #Use sigmoid activation function here. z0=sigmoid(a0)

model.add(Dense(1)) #define the output layer with one node with a linear activation z1=W1.z0+b1

print('Model Summary')
model.summary() #Prints the model layers summary

model.compile(loss='mean_squared_error',optimizer='adam') 
model.fit(X,Y,epochs=10000) #Train the model
